...
### Added
- `template/config/setup/` directory consolidates per-repo runtime conf
  files consumed by `setup.sh`. Ships four new confs — `gpu.conf`,
  `gui.conf`, `network.conf`, `volumes.conf` — plus the existing
  `image_name.conf` moved here.
- Per-repo overrides live at `<repo>/config/setup/<name>.conf` (mirrors
  template structure). Lookup order per conf: `<NAME>_CONF` env var →
  `<repo>/config/setup/<name>.conf` → template default.
- `setup.sh` new helpers: `_load_conf_value`, `_load_conf_lines`,
  `_resolve_conf_path`, `_load_gpu_conf` / `_load_gui_conf` /
  `_load_network_conf` / `_load_volumes_conf`, `_resolve_gpu` /
  `_resolve_gui`, and `generate_compose_yaml` — the last of which now
  emits a full `compose.yaml` on every run with conditional GPU
  `deploy` block, GUI env/volumes, and extra volumes from
  `volumes.conf`.
- `init.sh --gen-conf <name>` copies any template conf to
  `<repo>/config/setup/<name>.conf`. `--gen-image-conf` kept as an
  alias for back-compat.
- 48 new tests (295 → 343 total): 26 unit tests in `setup_spec.bats`,
  14 tests in new `compose_gen_spec.bats`, 8 integration tests in
...
- `image_name.conf` moved from `template/config/` to
  `template/config/setup/`. Per-repo override path changed from
  `<repo>/image_name.conf` to `<repo>/config/setup/image_name.conf`.
  No downstream repo has a per-repo override currently, so this is a
  clean break.
- **`compose.yaml` is now a derived artifact** generated by
  `setup.sh` on every run (alongside `.env`). `init.sh`-created
  `.gitignore` now includes `compose.yaml`. Structure is stable; values
  come from `config/setup/*.conf` + system detection. Users can inspect
  the file for a complete view of current runtime config.
- `init.sh` no longer writes a bare `compose.yaml` heredoc; it triggers
  `setup.sh` at the end of the scaffold flow so the initial
  `compose.yaml` is the real thing.
