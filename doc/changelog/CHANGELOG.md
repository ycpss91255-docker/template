...
### Added
- **`setup.sh` git-style subcommand dispatcher** (#49 Phase B-1). New explicit subcommands: `apply` (regenerate `.env` + `compose.yaml`, current default behaviour) and `check-drift` (compare current state against `.env`'s `SETUP_*` metadata, exit 0 when in sync / 1 when drift detected). Legacy flag-only invocation (`./setup.sh --base-path X --lang Y`) still works — it falls through to `apply` for backward compat. Future subcommands (`set` / `add` / `remove` / `show` / `list` / `reset`) will land in subsequent B-x PRs.
- **`setup.sh set` / `show` / `list` subcommands** (#49 Phase B-2). `set <section>.<key> <value>` writes to `setup.conf` via the same `_upsert_conf_value` helper the TUI uses (creates section / key on demand). Typed keys validate against `_tui_conf.sh` validators — `deploy.gpu_count` (gpu_count), `volumes.mount_*` (mount), `devices.cgroup_rule_*` (cgroup_rule), `network.port_*` (port_mapping), `environment.env_*` (env_kv), `resources.shm_size` (shm_size); unknown sections / invalid values exit 2 with i18n'd stderr. `set` does NOT regenerate `.env` — invoke `apply` explicitly for that. `show <section>.<key>` prints a single value, `show <section>` dumps all keys in on-disk order; missing key / section exits 1. `list` (no arg) prints the full setup.conf as INI sections (comments stripped) for tooling consumption; `list <section>` aliases `show <section>` for now.
- **`setup.sh add` / `remove` subcommands** (#49 Phase B-3). `add <section>.<list> <value>` appends to a list-style section by picking the next available numeric slot — first reuses any slot whose value is empty (matches the TUI's `_edit_list_section` placeholder-fill behaviour), otherwise uses `max+1`. Validators fire through the same `_setup_validate_kv` table B-2 set up. `remove <section>.<key>` deletes an exact key; `remove <section>.<list> <value>` finds the first `<list>_*` whose value matches and deletes that key (one entry per call — repeat to peel further). Comments and the rest of the file are preserved verbatim via `_write_setup_conf`. Neither subcommand regenerates `.env`.

### Changed
- **`build.sh` / `run.sh` no longer `source` `setup.sh`** (#49 Phase B-1, structurally closes the bug class behind #101). Drift detection now runs as a subprocess: `bash setup.sh check-drift --base-path "${FILE_PATH}" --lang "${_LANG}"`. The previous `source` callsite (used to obtain `_check_setup_drift` directly) is gone. This means new helpers added to `setup.sh` can no longer accidentally shadow caller-side functions even if they skip the `_setup_*` prefix convention. Four structural guards added to `template_spec.bats` (no-`source` + uses-`check-drift`, mirrored for `build.sh` / `run.sh`).

### Fixed
- **`setup_tui.sh` aborts on whiptail-only hosts** (closes #136). `_tui_run` hardcoded dialog's flag spellings (`--ok-label` / `--cancel-label`) which whiptail rejects with `unknown option`, breaking the very first menu on Ubuntu 22.04 minimal / Jetson arm64 (no `dialog` package). `_tui_backend.sh` now translates the spelling per `${TUI_BACKEND}`: dialog still gets `--ok-label` / `--cancel-label` / `--extra-button` / `--extra-label`; whiptail gets `--ok-button` / `--cancel-button` and skips the extra-button block entirely (whiptail has no third button at all). To preserve the Save & Exit affordance on whiptail, `_render_main_menu` injects a synthetic `__save` menu entry (i18n'd in all 4 languages) when `TUI_BACKEND == "whiptail"`. 7 regression tests added in `tui_backend_spec.bats` covering both backends across `_tui_msgbox`, `_tui_inputbox`, `_tui_menu`, and `_tui_run`.
- **`_msg` shadow bug after sourcing `setup.sh`** (closes #101). `build.sh` / `run.sh` used to source `setup.sh` to obtain `_check_setup_drift`; `setup.sh`'s top-level `_msg()` (with only 3 keys: `env_done` / `env_comment` / `unknown_arg`) silently shadowed the caller's richer `_msg()` (with `drift_regen` / `err_no_env` / `err_rerun_setup`). `_msg drift_regen` then returned empty and `printf "%s\n" ""` ate the drift-regen status line on every fresh-host / setup.conf-changed run (Jetson headless first surfaced it via `GUI_DETECTED: true → false`). Defensive fix: rename `setup.sh`'s `_msg()` → `_setup_msg()` (3 internal callers updated). Future helpers added to `setup.sh` should follow the `_setup_*` prefix convention. Two regression tests added to `template_spec.bats`: a static guard (`setup.sh` declares `_setup_msg`, not `_msg`) and a behavioral one (after `source setup.sh`, the caller's `_msg drift_regen` still resolves). B-1 above structurally retires the entire `source` pattern as a follow-on belt-and-braces fix.

### Changed
- **`init.sh` now scaffolds `main.yaml` with `permissions: contents: write`** (closes #62). New downstream repos generated by `./template/init.sh` get the permission block by default, so their first release tag push doesn't 403 at `softprops/action-gh-release` (ros1_bridge v1.5.0 hit this — caller-level permission grant is required because reusable workflow permissions intersect with the caller's, and GitHub's default GITHUB_TOKEN is read-only). Existing downstream repos must add the block manually one-time. Regression test added in `init_new_repo_spec.bats`.
