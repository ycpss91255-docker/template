...
Bootstrapping replaces ${TEMPLATE_REL}/ with a git subtree, and git
refuses to create one over a directory that still exists. Only tracked
files are removed for you, so the files above would keep that directory
alive and the run would fail halfway through.

${TEMPLATE_REL}/.env, ${TEMPLATE_REL}/compose.yaml and
${TEMPLATE_REL}/.env.generated are generated by 'just setup', 'just
build' and 'just run'. If you ran one of those before bootstrapping,
they are rebuilt on demand -- delete them and run ./${SCRIPT_NAME}
again. Anything above you want to keep, move it out of
${TEMPLATE_REL}/ first: bootstrapping replaces that directory wholesale."
}
...
_bootstrap_exit_trap() {
  ...
  _log_err "bootstrap failed after step 2 had already committed (exit ${_status}).
