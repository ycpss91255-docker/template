...
_create_new_repo() {
  ...
  # config/setup/*.conf rules; see _call_setup at the end of this flow.
...
YAML
  ...
  # .gitignore (compose.yaml is regenerated by setup.sh each build/run)
  cat > .gitignore <<'GIT'
.env
compose.yaml
coverage/
...
# ── Generate per-repo setup confs (image_name/gpu/gui/network/volumes) ──────

# _gen_conf <name>
#
# Copies template/config/setup/<name>.conf to <repo>/config/setup/<name>.conf
# so the user can customize that dimension for this repo. Refuses to
# overwrite an existing per-repo conf.
_gen_conf() {
  local _name="${1:?"${FUNCNAME[0]}: missing conf name"}"
  local _src="${TEMPLATE_DIR}/config/setup/${_name}.conf"
  local _dst_dir="${REPO_ROOT}/config/setup"
  local _dst="${_dst_dir}/${_name}.conf"
  if [[ ! -f "${_src}" ]]; then
    _error "Template ${_name}.conf not found at ${_src}"
  fi
  if [[ -f "${_dst}" ]]; then
    _error "${_name}.conf already exists at ${_dst}. Remove it first or edit directly."
  fi
  mkdir -p "${_dst_dir}"
  cp "${_src}" "${_dst}"
  _log "Created ${_dst}"
  _log "Edit it to customize ${_name} settings for this repo."
}

# Backward-compatible alias: --gen-image-conf behaves identically to
# --gen-conf image_name, kept to avoid breaking existing user scripts.
_gen_image_conf() {
  _gen_conf "image_name"
}
...
# _call_setup
#
# Runs setup.sh in the repo root so .env and compose.yaml are generated
# after the skeleton is in place. Both files are gitignored derived
# artifacts; conf files in config/setup/ are the source of truth.
_call_setup() {
  local _setup="${TEMPLATE_DIR}/script/docker/setup.sh"
  if [[ ! -x "${_setup}" ]] && [[ ! -f "${_setup}" ]]; then
  ...
  if ! bash "${_setup}" --base-path "${REPO_ROOT}" >/dev/null; then
    _log "WARNING: setup.sh exited non-zero; inspect manually and rerun ./build.sh"
  fi
}
...
main() {
  if [[ "${1:-}" =~ ^(-h|--help)$ ]]; then
    cat >&2 <<'EOF'
Usage: ./template/init.sh [--gen-conf <name> | --gen-image-conf]
...
Options:
  --gen-conf <name>     Copy template/config/setup/<name>.conf to
                        <repo>/config/setup/<name>.conf for per-repo
                        override. <name> is one of: image_name, gpu, gui,
                        network, volumes.
  --gen-image-conf      Alias for --gen-conf image_name (back-compat).
...
EOF
  ...
  if [[ "${1:-}" == "--gen-conf" ]]; then
    local _name="${2:?"--gen-conf requires a conf name (image_name|gpu|gui|network|volumes)"}"
    _gen_conf "${_name}"
    return 0
  fi
